...
void TopNExecutor::Init() {
  ...
  std::priority_queue<Tuple, std::vector<Tuple>, decltype(cmp)> pq(cmp);//自定义的大根堆

  Tuple child_tuple{};
  RID child_rid;
  //小根堆只有N 个元素
  while (child_->Next(&child_tuple, &child_rid)) {//获得一个child_tuple
    pq.push(child_tuple);
...
auto TopNExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  *tuple = child_tuples_.top();//获取一个tuple
  *rid = tuple->GetRid();
  child_tuples_.pop();
