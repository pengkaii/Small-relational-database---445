...
class LockManager {
 public:
  ...
  auto Dfs(txn_id_t txn_id) -> bool {
    //做了个记忆化搜索优化
    if (safe_set_.find(txn_id) != safe_set_.end()) {//安全集合里面找到了，不可能发生死锁，直接返回
      return false;
    }
    active_set_.insert(txn_id);

    std::vector<txn_id_t> &next_node_vector = waits_for_[txn_id];//邻接点数组，从之前的构件图里面取
    std::sort(next_node_vector.begin(), next_node_vector.end());
    //访问邻接点
    for (txn_id_t const next_node : next_node_vector) {
      if (active_set_.find(next_node) != active_set_.end()) { //如果邻接点在活跃集合里面，说明又收到它的顶点，出现死锁
        return true;
      }
      if (Dfs(next_node)) {
        return true;
    ...
    //表示索引邻接点没有找到环，不应该在活跃集合，要删掉，进入到安全的集合里面
 ...
 private:
  ...
  std::unordered_map<table_oid_t, std::shared_ptr<LockRequestQueue>> table_lock_map_;//表锁哈希表
  /** Coordination */
  std::mutex table_lock_map_latch_;

  /** Structure that holds lock requests for a given RID */
  std::unordered_map<RID, std::shared_ptr<LockRequestQueue>> row_lock_map_;//行锁哈希表
  /** Coordination */
  std::mutex row_lock_map_latch_;

  std::atomic<bool> enable_cycle_detection_;  //是否允许环检测
  std::thread *cycle_detection_thread_;
  /** Waits-for graph representation. */
  //等待图 顶点是事务  边表示依赖 T1指向T2 表示T1等待T2的锁释放
  std::unordered_map<txn_id_t, std::vector<txn_id_t>> waits_for_;
  std::mutex waits_for_latch_;

  std::set<txn_id_t> safe_set_;//安全集合
  std::set<txn_id_t> txn_set_; //事务ID集合
  std::unordered_set<txn_id_t> active_set_;//活跃事务集合

  std::unordered_map<txn_id_t, RID> map_txn_rid_;
  std::unordered_map<txn_id_t, table_oid_t> map_txn_oid_;  //锁  未授权集合
};
